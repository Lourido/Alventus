...
class ProjectDuplicateWizard(models.TransientModel):
    ...
    def action_duplicate(self):
        ...
        # 2. Crear el proyecto nuevo desde cero (SIN notificaciones)
        ...
        # Copiar adjuntos a nivel de PROYECTO
        ...
        # 3. Crear las nuevas etapas con fechas ajustadas (SIN notificaciones)
        ...
        for old_stage in original_stages:
            ...
            new_stage = self.env['project.task.type'].with_context(
                tracking_disable=True,
                mail_create_nolog=True,
            ).create({
                'name': new_stage_name,
        ...
        # 7. Asignar usuarios silenciosamente y mapear IDs (SIN notificaciones)
        ...
        # 8. Leer adjuntos en BULK (OPTIMIZACION CLAVE: 1 sola consulta SQL)
        ...
        # 9. Crear adjuntos en BULK (SIN notificaciones)
        ...
        # 10. Copiar archivos de ruta con sus descripciones (SIN notificaciones)
        step10_start = time.time()
        for route_file in original.route_file_ids:
            route_file.with_context(
                tracking_disable=True,
                mail_create_nolog=True,
            ).copy({
                'project_id': new_project.id,
        ...
        _logger.info("[PERF] TIEMPO TOTAL DE DUPLICACION: %.4fs", time.time() - total_start)
